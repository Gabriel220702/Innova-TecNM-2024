...
if (isset($_POST['submit'])) {
    ...
    if (empty($email) || empty($password)) {
        ...
        if ($resulte->num_rows === 0) {
            ...
            $sql = "INSERT INTO register (email, Password, Username) VALUES (?, ?, ?)";
            $stmt = $conx->prepare($sql);
            $stmt->bind_param("sss", $email, $password, $username);
            $stmt->execute();

            // Insertar registro en la tabla intentos_login si el registro de usuario es exitoso
            if ($stmt->affected_rows === 1) {
                // Solo registrar los intentos si el usuario se registró correctamente
                $stmt_insert_intentos = $conx->prepare("INSERT INTO intentos_login (correo, ip_usuario, intentos_fallidos, ultimo_intento) VALUES (?, ?, 0, NOW())");
            ...
            if (password_verify($password, $user['Password'])) {
                ...
                if ($bloqueado_hasta && (strtotime($bloqueado_hasta) > time())) {
                    ...
                    if ($intentos_fallidos < $max_intentos) {
                        ...
                        $stmt_reset_intentos = $conx->prepare("UPDATE intentos_login SET intentos_fallidos = 0 WHERE correo = ? AND ip_usuario = ?");
    ...
    <div class="container">
        ...
        <div class="panels-container">
            <div class="panel left-panel">
                <div class="content">
                    <h3>¿Aún no eres parte de Amoxtli-Jap?</h3>
                ...
                <img src="img/Logo_CinturonCCTN.png" class="image" alt="" />
            </div>
        </div>
    </div>
